...
struct  ENTRY *CIniFile::FindSection (int sectIndex)
{
	if (sectIndex >= GetSectCount())
	{
		return NULL;
	}
...
int CIniFile::GetSectKeyCount(int sectIndex)
{
	struct ENTRY *pEntry = FindSection(sectIndex);
	if (pEntry == NULL)	return 0;
	
	int keyCount = 0;
	struct ENTRY *pKeyEntry = pEntry->pNext;
	while (pKeyEntry != NULL)
	{
		if (pKeyEntry->Type == tpSECTION)
		{
			//已经到下一个section
			break;
		}
	...
	//获得指定sect的名字
CCHR * CIniFile::GetSectionName(int sectIndex)
{
	struct ENTRY *pEntry = FindSection(sectIndex);
...
//获得key的名字
CCHR * CIniFile::GetKeyName(int sectIndex, int keyIndex)
{
	int keyCount = GetSectKeyCount(sectIndex);
	...
	while (pKeyEntry != NULL)
	{
		...
		if (pKeyEntry->Type == tpKEYVALUE)
		{
			if (keyIndex == curIndex)
			{
				//找到指定的key 
				static char Text[255] = {0};
				strcpy (Text, pKeyEntry->pText);
				char *pText = strchr (Text, '=');
				if (pText != NULL)
				{
					//截掉'='后面的
					*pText = 0;
